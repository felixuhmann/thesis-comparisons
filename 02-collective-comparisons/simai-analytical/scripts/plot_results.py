#!/usr/bin/env python3
"""Generate plots from a SimAI-Analytical sweep's summary.csv.

Reads `output/<sweep>/summary.csv` (default: output/sweep_latest/) and writes
PNGs into `output/<sweep>/plots/`.

What gets produced:
  Per-collective (for all_reduce, all_gather, reduce_scatter):
    <coll>_time.png     log-log time vs payload size, NCCL meas + SimAI sim
    <coll>_busbw.png    busbw vs payload size (lin y, log x), same legend

  Cross-collective overviews:
...
Mirrors the AstraSim sibling project's plot style so the two simulators can
be eyeballed apples-to-apples.
"""
...
COL_NCCL = "#1f77b4"   # measured (NCCL, out-of-place)
COL_SIM  = "#d62728"   # SimAI sim (warm red)
...
def _format_size_axis(ax) -> None:
    ...
    ticks = [1 << k for k in (10, 14, 18, 22, 26, 30, 33)]  # 1KiB .. 8GiB
    ax.set_xticks(ticks)
...
def plot_time(df: pd.DataFrame, collective: str, out_path: Path, title_suffix: str) -> None:
    ...
    ax.plot(sub.size_bytes, sub.sim_time_us, "s-", color=COL_SIM,
            label="SimAI simulated")

    ax.set_yscale("log")
    _format_size_axis(ax)
    ax.set_ylabel("Time (\u00b5s)  \u2014 log scale")
    ax.set_title(f"{collective}: time vs payload size  {title_suffix}")
    ax.legend(loc="upper left")
    fig.savefig(out_path)
    plt.close(fig)


def plot_busbw(df: pd.DataFrame, collective: str, out_path: Path, title_suffix: str) -> None:
    sub = df[df.collective == collective].sort_values("size_bytes")
    fig, ax = plt.subplots()
    ...
    ax.plot(sub.size_bytes, sub.sim_busbw_GBs, "s-", color=COL_SIM,
            label="SimAI bus bandwidth")

    _format_size_axis(ax)
    ax.set_ylabel("Bus bandwidth (GB/s)")
    ax.set_ylim(bottom=0)
    ax.set_title(f"{collective}: bus bandwidth vs payload size  {title_suffix}")
    ax.legend(loc="upper left")
    fig.savefig(out_path)
    plt.close(fig)


def plot_overview_time(df: pd.DataFrame, out_path: Path, title_suffix: str) -> None:
    fig, ax = plt.subplots()
    ...
    _format_size_axis(ax)
    ax.set_ylabel("Time (\u00b5s) \u2014 log scale")
    ax.set_title(f"Sim vs measured time across modelled collectives  {title_suffix}")
    ax.legend(loc="upper left", ncol=2)
    fig.savefig(out_path)
    plt.close(fig)


def plot_overview_busbw(df: pd.DataFrame, out_path: Path, title_suffix: str) -> None:
    fig, ax = plt.subplots()
    for coll in SIM_COLLECTIVES:
        sub = df[df.collective == coll].sort_values("size_bytes")
        c = COLLECTIVE_PALETTE[coll]
    ...
    ax.set_ylabel("Bus bandwidth (GB/s)")
    ax.set_ylim(bottom=0)
    ax.set_title(f"Sim vs measured bus bandwidth across modelled collectives  {title_suffix}")
    ax.legend(loc="upper left", ncol=2)
    fig.savefig(out_path)
    plt.close(fig)


def plot_error_pct(df: pd.DataFrame, out_path: Path, title_suffix: str) -> None:
    fig, ax = plt.subplots()
    for coll in SIM_COLLECTIVES:
        sub = df[df.collective == coll].sort_values("size_bytes")
    ...
    for thresh in (-50, -30, -10, 10, 30, 50):
        ax.axhline(thresh, color="lightgrey", linewidth=0.6, linestyle=":")
    _format_size_axis(ax)
    ax.set_ylabel("(sim - measured) / measured  [%]")
    ax.set_title(f"SimAI time error % vs measured  {title_suffix}")
    ax.legend(loc="lower right")
    fig.savefig(out_path)
    plt.close(fig)


def generate_all(csv_path: Path, plots_dir: Path, title_suffix: str) -> list[Path]:
    ...
    for coll in SIM_COLLECTIVES:
        ...
        plot_time(df, coll, t, title_suffix)
        plot_busbw(df, coll, b, title_suffix)
        produced += [t, b]
    ...
    plot_overview_time(df, over_t, title_suffix)
    plot_overview_busbw(df, over_b, title_suffix)
    plot_error_pct(df, err, title_suffix)
    produced += [over_t, over_b, err]
    return produced
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--csv",
        type=Path,
        default=results_dir / "output" / "sweep_latest" / "summary.csv",
        help="Path to a sweep's summary.csv (default: output/sweep_latest/summary.csv).",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    parser.add_argument(
        "--title-suffix",
        type=str,
        default="(4\u00d7H100, intra-node, NV6)",
        help="Suffix appended to every plot title; useful for tagging calibration runs.",
    )
...
def main(argv: list[str]) -> int:
    ...
    _set_style()
    produced = generate_all(args.csv, out_dir, args.title_suffix)
